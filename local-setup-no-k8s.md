# RouteGate - Local Setup without Kubernetes

This guide explains how to deploy RouteGate on your local machine using Docker only, with no Kubernetes required. The entire process takes approximately 5-10 minutes.
...
If the page does not load, check that all containers are running:

```bash
docker ps --filter "network=routegate-net"
```

You should see `mysql`, `weather-service`, `api-service`, and `frontend` all with status `Up`.
